...
namespace ShareX.HelpersLib
{
    public static partial class NativeMethods
    {
        ...
        public static bool SetWindowCaptionColor(IntPtr handle, int colorRef)
        {
            if (Helpers.IsWindows10OrGreater(22000))
            {
                return DwmSetWindowAttribute(handle, (int)DWMWINDOWATTRIBUTE.DWMWA_CAPTION_COLOR, ref colorRef, sizeof(int)) == 0;
            }

            return false;
        }

        public static bool SetWindowTextColor(IntPtr handle, int colorRef)
        {
            if (Helpers.IsWindows10OrGreater(22000))
            {
                return DwmSetWindowAttribute(handle, (int)DWMWINDOWATTRIBUTE.DWMWA_TEXT_COLOR, ref colorRef, sizeof(int)) == 0;
            }

            return false;
        }

        public static void ApplyDarkTitleBar(IntPtr handle, int captionColorRef = 0x00222222, int textColorRef = 0x00FFFFFF)
        {
            if (handle != IntPtr.Zero)
            {
                UseImmersiveDarkMode(handle, true);
                SetWindowCaptionColor(handle, captionColorRef);
                SetWindowTextColor(handle, textColorRef);
            }
        }
