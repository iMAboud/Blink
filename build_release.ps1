...
dotnet publish (Join-Path $rootDir "ShareX\ShareX.csproj") -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:DebugType=none -p:DebugSymbols=false -p:EnableWindowsTargeting=true -o $appStagingDir
if ($LASTEXITCODE -ne 0) {
    throw "ShareX publish failed."
}

Write-Host "Publishing Blink Updater..." -ForegroundColor Cyan
dotnet publish (Join-Path $rootDir "ShareX.Updater\ShareX.Updater.csproj") -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true -p:DebugType=none -p:DebugSymbols=false -p:EnableWindowsTargeting=true -o $appStagingDir
if ($LASTEXITCODE -ne 0) {
...
dotnet publish (Join-Path $rootDir "ShareX.Launcher\ShareX.Launcher.csproj") -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:DebugType=none -p:DebugSymbols=false -p:EnableWindowsTargeting=true -o (Join-Path $rootDir "ShareX.Launcher\publish")
if ($LASTEXITCODE -ne 0) {
