        ...
        public async Task UseCommandLineArgs(List<CLICommand> commands)
        {
            if (commands != null && commands.Count > 0)
            {
                ...
                foreach (CLICommand command in commands)
                {
                    ...
                    {
                        if (command.CheckCommand("import") && !string.IsNullOrEmpty(command.Parameter) && File.Exists(command.Parameter))
                        {
                            SettingManager.Import(command.Parameter);
                    ...
                    if (!string.IsNullOrEmpty(command.Command) && (command.Command.EndsWith(".sxb", StringComparison.OrdinalIgnoreCase) || command.Command.EndsWith(".bkb", StringComparison.OrdinalIgnoreCase)))
                    {
                        if (File.Exists(command.Command))
                        {
                            SettingManager.Import(command.Command);
                            SettingManager.LoadAllSettings();
                            await Program.MainForm.UpdateControls();
                        }
                        continue;
