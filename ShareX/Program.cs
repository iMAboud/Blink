...
namespace ShareX
{
    ...
    {
        public const string AppName = "Blink";
        public const string MutexName = "Blink-82E6AC09-0FEF-4390-AD9F-0DD3F5561EFC";
        ...
        public static string ScreenshotsParentFolder
        {
            get
            {
                ...
                return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures), "Blink");
            }
        }
        ...
        private static async Task UseCommandLineArgs(string[] args)
        {
            ...
            else if (MainWindowIntegration.IsVisible || SettingsIntegration.IsVisible)
            {
                SettingsIntegration.Show();
            }
        ...
        private static void UpdatePersonalPath()
        {
            ...
            {
                Portable = true;
                string parentLauncher = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", $"{AppName}.exe");
                string legacyParentLauncher = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "ShareX.exe");
                string parentBlinkFolder = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", AppName));
                string localBlinkFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, AppName);

                if (File.Exists(Path.Combine(parentBlinkFolder, SettingManager.ApplicationConfigFileName)))
                {
                    CustomPersonalPath = parentBlinkFolder;
                }
                else if (File.Exists(Path.Combine(localBlinkFolder, SettingManager.ApplicationConfigFileName)))
                {
                    CustomPersonalPath = localBlinkFolder;
                }
                else if (File.Exists(parentLauncher) || File.Exists(legacyParentLauncher))
                {
                    CustomPersonalPath = parentBlinkFolder;
                }
                else
                ...
                PersonalPathDetectionMethod = "Default portable";
        ...
        public static void UpdateHelpersSpecialFolders()
        {
            ...
            specialFolders.Add($"{AppName}ImageEffects", ImageEffectsFolder);
