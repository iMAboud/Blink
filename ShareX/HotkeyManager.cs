...
namespace ShareX
{
    public class HotkeyManager
    {
        ...
        public void UnregisterAllHotkeys(bool removeFromList = false, bool temporary = false)
        {
            if (Hotkeys != null)
            {
                foreach (HotkeySettings hotkeySetting in Hotkeys.ToArray())
        ...
        public void ShowFailedHotkeys()
        {
            ...
            if (failedHotkeysList.Count > 0)
            {
                ...
                MessageBox.Show(text, "Blink - " + Strings.HotkeyManager_ShowFailedHotkeys_Hotkey_registration_failed, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        {
            HotkeysConfig defaultHotkeys = SettingManager.LoadDefaultSetting<HotkeysConfig>("DefaultHotkeysConfig.json");
            if (defaultHotkeys?.Hotkeys != null && defaultHotkeys.Hotkeys.Count > 0)
            {
                return defaultHotkeys.Hotkeys;
            }

            HotkeySettings regionHotkey = new HotkeySettings(HotkeyType.RectangleRegion, Keys.PrintScreen);
            regionHotkey.TaskSettings.UseDefaultAfterCaptureJob = false;
            regionHotkey.TaskSettings.AfterCaptureJob = AfterCaptureTasks.SaveImageToFile | AfterCaptureTasks.CopyImageToClipboard;

            HotkeySettings fullScreenHotkey = new HotkeySettings(HotkeyType.PrintScreen, Keys.Control | Keys.PrintScreen);
            fullScreenHotkey.TaskSettings.UseDefaultAfterCaptureJob = false;
            fullScreenHotkey.TaskSettings.AfterCaptureJob = AfterCaptureTasks.SaveImageToFile | AfterCaptureTasks.CopyImageToClipboard;

            HotkeySettings windowHotkey = new HotkeySettings(HotkeyType.ActiveWindow, Keys.Alt | Keys.PrintScreen);
            windowHotkey.TaskSettings.UseDefaultAfterCaptureJob = false;
            windowHotkey.TaskSettings.AfterCaptureJob = AfterCaptureTasks.SaveImageToFile | AfterCaptureTasks.CopyImageToClipboard;

            HotkeySettings gifHotkey = new HotkeySettings(HotkeyType.ScreenRecorderGIF, Keys.Control | Keys.Shift | Keys.PrintScreen);
