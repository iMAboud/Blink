...
namespace ShareX
{
    public static class TaskHelpers
    {
        ...
        public static string CreateThumbnail(Bitmap bmp, string folder, string fileName, TaskSettings taskSettings)
        {
                ...
                (bmp.Width > taskSettings.ImageSettings.ThumbnailWidth && bmp.Height > taskSettings.ImageSettings.ThumbnailHeight)))
            {
                ...
                if (!string.IsNullOrEmpty(thumbnailFilePath))
                {
                    ...
                    using (Bitmap resizedImage = ImageHelpers.ResizeImage(thumbnail, taskSettings.ImageSettings.ThumbnailWidth, taskSettings.ImageSettings.ThumbnailHeight))
                ...
                if (!string.IsNullOrEmpty(input))
                {
                    ...
                    if (taskSettings.GeneralSettings.ShowToastNotificationAfterTaskCompleted)
                    {
                        ShowNotificationTip(string.Format(Strings.TaskHelpers_OpenQuickScreenColorPicker_Copied_to_clipboard___0_, text),
                            "Blink - " + Strings.ScreenColorPicker);
                    }
                }
            }
        }
        ...
        public static void PinToScreenFromClipboard(TaskSettings taskSettings = null)
        {
            ...
            {
                MessageBox.Show(Strings.ClipboardDoesNotContainAnImage, "Blink - " + Strings.PinToScreen, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        public static async Task<bool> CheckFFmpegAsync(TaskSettings taskSettings)
        {
            if (!Environment.Is64BitOperatingSystem && !taskSettings.CaptureSettings.FFmpegOptions.OverrideCLIPath)
            {
                MessageBox.Show(Strings.FFmpegOnlySupports64BitOperatingSystems,
                    "Blink - " + Strings.FFmpegIsMissing, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            {
                bool downloaded = await FFmpegDownloader.DownloadFFmpegAsync(ffmpegPath);
                if (!downloaded || !File.Exists(ffmpegPath))
                {
                    return false;
                }
            }
        ...
        {
            return CheckFFmpegAsync(taskSettings).GetAwaiter().GetResult();
        }

        public static void PlaySound(string soundPath, Stream defaultSound = null)
        {
            if (string.IsNullOrEmpty(soundPath) || soundPath.Equals("default", StringComparison.OrdinalIgnoreCase))
            {
                if (defaultSound != null)
                {
                    Helpers.PlaySoundSync(defaultSound);
                }
                return;
            }

            switch (soundPath.ToLowerInvariant())
            {
                case "builtin:capture":
                    Helpers.PlaySoundSync(Resources.CaptureSound);
                    break;
                case "builtin:task_completed":
                    Helpers.PlaySoundSync(Resources.TaskCompletedSound);
                    break;
                case "builtin:action_completed":
                ...
                default:
                    ...
                    {
                        Helpers.PlaySoundAsync(soundPath);
                    }
            ...
            switch (notificationSound)
            {
                case NotificationSound.Capture:
                    if (taskSettings.GeneralSettings.PlaySoundAfterCapture)
                    {
                        ...
                        {
                            PlaySound(taskSettings.GeneralSettings.CustomCaptureSoundPath, Resources.CaptureSound);
                        }
                        else
                        {
                            Helpers.PlaySoundSync(Resources.CaptureSound);
                ...
                case NotificationSound.TaskCompleted:
                    if (taskSettings.GeneralSettings.PlaySoundAfterUpload)
                    {
                        ...
                        {
                            PlaySound(taskSettings.GeneralSettings.CustomTaskCompletedSoundPath, Resources.TaskCompletedSound);
                        }
                        else
                        {
                            Helpers.PlaySoundSync(Resources.TaskCompletedSound);
                ...
                case NotificationSound.ActionCompleted:
                    if (taskSettings.GeneralSettings.PlaySoundAfterAction)
                    {
                        ...
                        {
                            PlaySound(taskSettings.GeneralSettings.CustomActionCompletedSoundPath, Resources.ActionCompletedSound);
                        }
                        else
                        {
                            Helpers.PlaySoundSync(Resources.ActionCompletedSound);
                ...
                case NotificationSound.Error:
                    if (taskSettings.GeneralSettings.PlaySoundAfterUpload)
                    {
                        ...
                        {
                            PlaySound(taskSettings.GeneralSettings.CustomErrorSoundPath, Resources.ErrorSound);
                        }
                        else
                        {
                            Helpers.PlaySoundSync(Resources.ErrorSound);
        ...
        public static async Task DownloadDevBuild()
        {
            ...
            {
                MessageBox.Show(Strings.ShareXIsUpToDate, "Blink", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        public static void ShowNotificationTip(string text, string title = "Blink", int duration = -1)
        {
            if (duration < 0)
            {
        ...
        public static bool IsUploadAllowed()
        {
            ...
            {
                MessageBox.Show(Strings.YourSystemAdminDisabledTheUploadFeature, "Blink", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                MessageBox.Show(Strings.ThisFeatureWillNotWorkWhenDisableUploadOptionIsEnabled, "Blink", MessageBoxButtons.OK, MessageBoxIcon.Information);
