...
public partial class TaskSettingsView : UserControl, ITaskSettingsHost
{
    ...
    public string? Title => (TopLevel.GetTopLevel(this) as Window)?.Title ?? Strings.Settings_TaskSettings;
    ...
    public TaskSettingsView() : this(Program.DefaultTaskSettings, true)
    {
    }

    public TaskSettingsView(TaskSettings settings, bool isDefault)
    {
        InitializeComponent();
    ...
    public void Reload()
    {
        _viewModel = new TaskSettingsViewModel(true);
        DataContext = _viewModel;
        SettingsPages.Children.Clear();
        _pages = new TaskSettingsPageBuilder(this, Program.DefaultTaskSettings, true).BuildPages();
        foreach (Control page in _pages.Values)
        {
            SettingsPages.Children.Add(page);
        }
    ...
    public void ShowNotificationButtonsEditor(
